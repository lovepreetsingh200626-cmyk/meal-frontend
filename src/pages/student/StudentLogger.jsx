...
export default function StudentLogger({ user }) {
  ...
  const fetchDateRecord = useCallback(async (date) => {
    if (!userId) return;
    setLoading(true); // Trigger full-page loader on date change or mount
    try {
      ...
      setTimeout(() => setLoading(false), 400); // Small artificial delay for visual transition
    }
  }, [userId]);
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 pb-16 font-sans selection:bg-blue-950 selection:text-white flex flex-col">
      ...
      {/* 1. TOP FULL-WIDTH EMBLEM & STATUTORY STRIP */}
      <div className="bg-slate-950 text-slate-300 text-[10px] font-bold px-4 md:px-8 py-2 border-b-2 border-orange-600 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-1 shadow-xs select-none print:hidden">
        <div className="flex items-center gap-2 uppercase tracking-widest text-slate-200">
          <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
          <span className="truncate">Central Hostel Cooperative Registry</span>
        </div>
        <div className="flex items-center gap-2 text-[8px] sm:text-[9px] font-mono uppercase tracking-wider text-slate-400">
          <span>Statute 4.2 • <strong className="text-white">Verified Log</strong></span>
        </div>
      </div>

      <main className="max-w-5xl mx-auto px-3 sm:px-6 w-full space-y-5">
        {loading ? (
            <div className="flex flex-col items-center justify-center min-h-[65vh] bg-white border-2 border-slate-300 shadow-sm border-t-4 border-t-blue-950 w-full animate-in fade-in duration-300 mt-5">
                <Loader2 className="w-10 h-10 animate-spin text-orange-600 mb-4" />
                <p className="text-[11px] font-mono font-black uppercase tracking-widest text-slate-600">Accessing Daily Diet Ledger...</p>
            </div>
        ) : (
                ...
                <div className="bg-white border border-slate-300 shadow-sm p-5 sm:p-7 border-t-4 border-t-blue-950">
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-200 pb-4 mb-6 gap-4">
                        <div className="flex items-start gap-3">
                            <div className="p-2 sm:p-2.5 bg-blue-950 text-orange-400 border border-blue-900 shrink-0 mt-0.5 shadow-xs">
                                <Landmark className="w-5 h-5 sm:w-6 sm:h-6" />
                            </div>
                            <div>
                                <div className="flex items-center gap-2 flex-wrap">
                                    <span className="text-[8px] sm:text-[9px] font-black uppercase bg-slate-100 text-slate-700 border border-slate-300 px-2 py-0.5 font-mono">
                                        Official Record
                                    </span>
                                </div>
                                <h2 className="text-sm sm:text-lg font-black text-blue-950 uppercase tracking-tight font-serif mt-1">
                        ...
                        <div className="text-left sm:text-right w-full sm:w-auto">
                            <span className="text-[9px] font-black uppercase bg-orange-50 text-orange-900 border border-orange-300 px-2.5 py-1 tracking-wider inline-block">
                                Statute 4.2 • Minimum Quota Enforced
                            </span>
                        </div>
                    </div>
                    ...
                    {errorMsg && (
                        <div className="bg-orange-50 border border-orange-300 border-l-4 border-l-orange-600 text-orange-950 p-3.5 text-xs font-bold uppercase mb-5 flex items-start gap-3 shadow-xs">
                            ...
                            <div>
                                ...
                                <p className="text-[11px] font-medium text-orange-900 mt-0.5 normal-case">{errorMsg}</p>
                            </div>
                        </div>
                    )}
                    {successMsg && (
                    ...
                    <div className="bg-slate-50 border border-slate-300 p-4 mb-6 shadow-xs">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                            ...
                            <input
                                ...
                                className="w-full sm:w-auto border-2 border-slate-400 p-2 text-xs font-mono font-bold text-slate-950 outline-none focus:border-blue-950 bg-white cursor-pointer"
                            />
                        </div>
                    </div>

                    {/* STATUTORY MEAL TILES */}
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 mb-3">
                            ...
                            <div className="flex gap-2 shrink-0">
                                ...
                                <button
                                    ...
                                    className="bg-blue-950 hover:bg-blue-900 text-white font-black px-4 py-2 text-[10px] uppercase tracking-wider cursor-pointer transition flex items-center gap-1 border-b border-orange-500 active:scale-95 shadow-xs"
                    ...
                    {/* COMPUTED STATUTORY LEVY ASSESSMENT BOX WITH INSTITUTIONAL ORANGE THEME */}
                    <div className="bg-slate-100 border-2 border-slate-300 p-5 mb-6">
                        ...
                        {liveCalc.mealCount === 1 && (
                            <div className="mt-3.5 pt-3 border-t border-orange-300 flex items-start gap-2.5 text-orange-950 bg-orange-50/90 p-3 border-l-4 border-l-orange-600 shadow-xs">
                                <AlertTriangle className="w-4 h-4 text-orange-600 shrink-0 mt-0.5" />
                                <p className="text-[10px] font-bold uppercase leading-relaxed font-mono">
                                    Statute 4.2 Minimum Quota Assessment: Single-meal attendance incurs mandatory <span className="text-orange-700 font-black">₹{liveCalc.penaltyCost}</span> unselected meal charge appended to balance minimum residential diet requirement.
                                </p>
                            </div>
                        )}
                    </div>
                    ...
                    <button
                        ...
                        className="w-full bg-blue-950 hover:bg-blue-900 active:bg-blue-950 text-white font-black py-3.5 text-xs uppercase tracking-widest cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed transition-all shadow-sm border-b-2 border-orange-500 flex items-center justify-center gap-2"
                    >
                        {saving ? (
                            <>
